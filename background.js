...
async function handleCheckPhishing(url, sendResponse) {
	try {
		...
		console.log("[PhiLNet] Error checking phishing:", error);
		sendResponse({
			score: null,
			isPhishing: false,
		});
...
async function callPhishingAPI(url, source) {
	...
	try {
		...
		if (!response.ok) {
			console.log(`[PhiLNet] API responded with status: ${response.status}`);
			return false;
		}
		...
		console.log("[PhiLNet] API request failed:", err);
		return false;
	}
}
